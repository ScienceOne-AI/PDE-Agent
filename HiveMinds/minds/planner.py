...
from HiveMinds.context.build_context import build_subtask_prompt, build_re_subtask_prompt, build_subtask_prompt_zero
...
class Planner(Mind):
    ...
    def __init__(self, role: str, instruction: str, 
                 toolkit_info: Dict, model_info='deepseek-chat', 
    ...
    def analyze_query(self, 
                      ...
                      validation_info=None) -> str:
        # image_info = self.get_image_info(image)
        
        # FIXME: This is a temporary, need to refactor this part
        if not previous_actions:
            subtask_prompt = build_subtask_prompt(question, retrieval_case, self.toolkit_info).strip()
            # subtask_prompt = build_subtask_prompt_zero(question, self.toolkit_info).strip()
        else:
            # FIXME
            # adjust_execution_file(loop_count-1)
            subtask_prompt = build_re_subtask_prompt(question, retrieval_case, self.toolkit_info,
                                                    list(self.memory.current_info.values())[0], 
                                                    previous_actions, validation_info).strip()
        
        subtask_think, subtask_answer = self.chat(subtask_prompt)
        subtask_info = json.loads(subtask_answer)
        
        # Memory {'planning_task_v1': []}
        self.memory.add_memory_info({f'planning_task_v{len(self.memory.memory_info)}': subtask_info})
        
        # logging the subtask_info
        logging.info(f"Subtask Prompt: \n{subtask_prompt}")
        logging.info(f"Subtask Think: \n{subtask_think}")
        logging.info(f"Subtask Info Answer: \n{subtask_info}")
